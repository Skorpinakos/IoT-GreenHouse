...
const getRecents = (n, callback) => {
    ...
    let sql="SELECT DISTINCT GREENHOUSE_ID, PM.ID, TYPE, MEASUREMENT_DATE, MEASUREMENT_TIME, HEALTH, GREENHOUSE_PHOTO, MEASUREMENT_PHOTO FROM (PLANT AS P JOIN (SELECT * FROM PLANT_MEASUREMENT) AS PM on P.ID = PLANT_ID) JOIN GREENHOUSE AS G ON GREENHOUSE_ID = G.ID ORDER BY MEASUREMENT_DATE DESC, MEASUREMENT_TIME ASC LIMIT ?";
