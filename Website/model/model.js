...
const get_plant_info = (id, callback) => {
    ...
    let sql='SELECT GREENHOUSE_ID, PM.ID AS PM_ID, P.ID AS P_ID, TYPE, LIFESPAN, MEASUREMENT_DATE, MEASUREMENT_TIME, HEALTH, SIZE, GROWTH, ROW, COLUMN, MEASUREMENT_PHOTO  FROM (PLANT_MEASUREMENT AS PM  JOIN PLANT AS P ON PLANT_ID = P.ID) WHERE PLANT_ID=?  ORDER BY MEASUREMENT_DATE DESC, MEASUREMENT_TIME ASC LIMIT 2';
    const db = new sqlite3.Database(db_name);
