...
def check_cluster_multitudes(test_cases,edge_point_list):
    ...
    wcss = []
    for i in test_cases:
        kmeans = KMeans(n_clusters=i, init='k-means++', max_iter=130, n_init=8, random_state=0)  #increase max_iter and n_init for more accurate error results for elbow graph and elbow finding, decrease for faster 'find errors for test cases' time
        #print(edge_point_list)
...
def find_centroids(edge_point_list,n):
    kmeans = KMeans(n_clusters=n, init='k-means++', max_iter=130, n_init=8, random_state=0) #increase max_iter and n_init for better clustering , decrease for faster 'find final centroids' time
    kmeans.fit(edge_point_list)
